...
const WritingQuestion = require("../../../../../../models/TOEIC/question/writing/writingTOEIC.model");
...
const deleteFile = (filePath) => {
  ...
  try {
    ...
    normalizedPath = normalizedPath
      .replace(
        /^\/admin\/img\/Uploads_writing_TOEIC\//,
        "/shared/images/writing_TOEIC/"
      )
      .replace(
        /^\\shared\\images\\writing_TOEIC\\/,
        "/shared/images/writing_TOEIC/"
...
exports.createQuestion = async (req, res) => {
  try {
    const { MaCC, TopicN, part, questionN, difficulty, notes, ...bodyData } =
      req.body;
    ...
    if (![8, 9, 10].includes(partNum))
      throw new Error("Part không hợp lệ. Chỉ chấp nhận 8, 9, hoặc 10.");
    ...
    if (duplicate)
      throw new Error(`Câu hỏi Part ${partNum} số ${questionNum} đã tồn tại.`);
    ...
    switch (partNum) {
      case 8:
        if (
          !bodyData.keyword1?.trim() ||
          !bodyData.keyword2?.trim() ||
          !req.file
        ) {
      ...
      case 9:
        ...
        const sampleAnswerPart9 =
          typeof bodyData.sampleAnswer === "string"
            ? bodyData.sampleAnswer.trim()
            : "";
        questionData.part9 = {
