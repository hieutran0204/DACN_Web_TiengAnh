...
interface SubQuestion {
  ...
  options?: string[];
  type?: string; // Add type to subquestion
}
...
interface ReadingPassage {
  ...
  title: string;
  passage: string;
  type?: string; // Make optional as it might be on subquestion now
  subQuestions: SubQuestion[];
}

// Helper to get type
const getQuestionType = (passage: ReadingPassage, sq: SubQuestion) => {
  return sq.type || passage.type || "multiple_choice";
};
    ...
    try {
      ...
      if (res.success) {
        ...
        toast({
          ...
          description: `Score: ${res.data.score}/${res.data.totalQuestions} → Band ${band.toFixed(1)}`,
        });
      } else {
        toast({
          variant: "destructive",
