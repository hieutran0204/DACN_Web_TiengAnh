  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/chat", {
        ...
        body: JSON.stringify({
          message: inputValue,
          // Gemini requires history to start with 'user'. The initial AI greeting must be excluded.
          history: messages
            .filter(m => m.id !== "1") // Exclude the default welcome message
            .map(m => ({
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        text: "Sorry, I'm having trouble connecting to the AI server. Please try again later.",
        sender: "ai",
        timestamp: new Date(),
      }
