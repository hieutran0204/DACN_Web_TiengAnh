const Reading = require("../../../models/TOEIC/question/reading/readingToiec.model");
const Listening = require("../../../models/TOEIC/question/listening/listeningTOEIC.model");
...
exports.showExams = async (req, res) => {
  try {
    ...
    const partFilter =
      partArray.length > 0 ? { part: { $in: partArray.map(Number) } } : {};

    // Thực hiện query trên tất cả các model cần thiết
    const queryPromises = modelsToQuery.map((model) =>
      model.find(partFilter).exec()
    );
    ...
    results.forEach((exams, index) => {
      const skill =
        modelsToQuery[index] === Listening ? "listening" : "reading";
