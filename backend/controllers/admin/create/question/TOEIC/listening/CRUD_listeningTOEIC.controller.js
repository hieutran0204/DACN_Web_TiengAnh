...
const deleteFile = (filePath) => {
  ...
  ); // Loại bỏ /shared để khớp với cấu trúc thư mục
...
const getModelByPart = (part) => {
  switch (Number(part)) {
    case 1:
      return ListeningTOEICPart1;
    case 2:
      return ListeningTOEICPart2;
    case 3:
      return ListeningTOEICPart3;
    case 4:
      return ListeningTOEICPart4;
    default:
      throw new Error("Phần không hợp lệ");
  }
};
...
exports.createListeningQuestionPart1 = async (req, res) => {
  try {
    ...
    if (req.fileValidationError) {
      return res
        .status(400)
        .json({ error: `Lỗi upload file: ${req.fileValidationError}` });
    }
    ...
    if (!files || !files.audio || !files.audio[0]?.filename) {
      return res
        .status(400)
        .json({ error: "File audio là bắt buộc (MP3/WAV)" });
    }
    if (!files.image || !files.image[0]?.filename) {
      return res
        .status(400)
        .json({ error: "File hình ảnh là bắt buộc (JPEG/PNG/GIF/WEBP)" });
    }
    if (!paragraph?.trim()) {
      return res
        .status(400)
        .json({ error: "Câu hỏi mô tả hình ảnh là bắt buộc" });
    }
    if (
      !optionA?.trim() ||
      !optionB?.trim() ||
    ...
    if (!correctAnswer || !["A", "B", "C", "D"].includes(correctAnswer)) {
      return res
        .status(400)
        .json({ error: "Đáp án đúng phải là A, B, C hoặc D" });
    }
    ...
    if (duplicateQuestion) {
      ...
      return res.status(400).json({
        error: `Số câu hỏi ${questionNumber} đã tồn tại trong Part 1`,
      });
    }
    ...
    ["audio", "image"].forEach((field) => {
      if (req.files?.[field]?.[0]?.filename) {
        ...
        deleteFile(
          `/shared/${folder}/listening_TOEIC/${req.files[field][0].filename}`
        );
      }
    });
...
exports.createListeningQuestionPart2 = async (req, res) => {
  try {
    ...
    if (!optionA?.trim() || !optionB?.trim() || !optionC?.trim()) {
      return res
        .status(400)
        .json({ error: "Phải cung cấp đúng 3 đáp án (A, B, C)" });
    }
    ...
    if (duplicateQuestion) {
      uploadedFiles.forEach(deleteFile);
      return res.status(400).json({
        error: `Số câu hỏi ${questionNumber} đã tồn tại trong Part 2`,
      });
    }
    ...
    if (req.files?.audio?.[0]?.filename) {
      deleteFile(
        `/shared/audio/listening_TOEIC/${req.files.audio[0].filename}`
      );
    }
    res.status(500).json({ error: `Lỗi server: ${error.message}` });
  }
  ...
  try {
    ...
    const questions = [
      {
        ...
        options: [
          question1OptionA?.trim() || "",
          question1OptionB?.trim() || "",
          question1OptionC?.trim() || "",
          question1OptionD?.trim() || "",
        ],
      ...
      },
      {
        ...
        options: [
          question2OptionA?.trim() || "",
          question2OptionB?.trim() || "",
          question2OptionC?.trim() || "",
          question2OptionD?.trim() || "",
        ],
      ...
      },
      {
        ...
        options: [
          question3OptionA?.trim() || "",
          question3OptionB?.trim() || "",
          question3OptionC?.trim() || "",
          question3OptionD?.trim() || "",
        ],
    ...
    for (let i = 0; i < 3; i++) {
      if (!questions[i].text.trim()) {
        return res
          .status(400)
          .json({ error: `Nội dung câu hỏi ${i + 1} là bắt buộc` });
      }
      if (!questions[i].options.every((opt) => opt.trim())) {
        return res
          .status(400)
          .json({ error: `Câu hỏi ${i + 1} phải có đúng 4 đáp án` });
      }
      if (
        !questions[i].correctAnswer ||
        !["A", "B", "C", "D"].includes(questions[i].correctAnswer)
      ) {
        return res.status(400).json({
          error: `Đáp án đúng của câu hỏi ${i + 1} phải là A, B, C hoặc D`,
        });
      }
    }
    ...
    const questionData = {
      ...
      diagramUrl:
        files.diagram && files.diagram[0]?.filename
          ? `/shared/diagrams/listening_TOEIC/${files.diagram[0].filename}`
          : "",
      questions,
    };
    ...
    if (duplicateQuestion) {
      uploadedFiles.forEach(deleteFile);
      return res.status(400).json({
        error: `Số câu hỏi ${questionNumber} đã tồn tại trong Part 3`,
      });
    }
...
exports.createListeningQuestionPart4 = async (req, res) => {
  try {
    ...
    if (duplicateQuestion) {
      uploadedFiles.forEach(deleteFile);
      return res.status(400).json({
        error: `Số câu hỏi ${questionNumber} đã tồn tại trong Part 4`,
      });
    }
...
exports.updateQuestion = async (req, res) => {
  try {
    ...
    const newQuestionNumber =
      parseInt(questionNumber) || existingQuestion.questionNumber;
    ...
    if (duplicateQuestion) {
      return res.status(400).json({
        error: `Số câu hỏi ${newQuestionNumber} đã tồn tại trong Part ${existingQuestion.part}`,
      });
    }
    ...
    switch (existingQuestion.part) {
      case 1:
        ...
        if (!existingQuestion.imageUrl) {
          return res
            .status(400)
            .json({ error: "Hình ảnh là bắt buộc cho Part 1" });
        }
        if (
          !optionA?.trim() ||
          !optionB?.trim() ||
          ...
          return res
            .status(400)
            .json({ error: "Part 1 phải có đúng 4 lựa chọn" });
        }
        ...
        if (
          !correctAnswer ||
          !["A", "B", "C", "D"].includes(existingQuestion.correctAnswer)
        ) {
          return res
            .status(400)
            .json({ error: "Đáp án đúng phải là A, B, C hoặc D" });
      ...
      case 2:
        ...
        if (!existingQuestion.questionText) {
          return res
            .status(400)
            .json({ error: "Câu hỏi là bắt buộc cho Part 2" });
        }
        if (!optionA?.trim() || !optionB?.trim() || !optionC?.trim()) {
          return res
            .status(400)
            .json({ error: "Part 2 phải có đúng 3 lựa chọn" });
        }
        existingQuestion.options = [
          optionA.trim(),
          optionB.trim(),
        ...
        if (
          !correctAnswer ||
          !["A", "B", "C"].includes(existingQuestion.correctAnswer)
        ) {
          return res
            .status(400)
            .json({ error: "Đáp án đúng phải là A, B hoặc C" });
        }
        break;
      ...
      case 3:
        ...
        for (let i = 0; i < 3; i++) {
          if (!questionsPart3[i].text.trim()) {
            return res.status(400).json({
              error: `Câu hỏi ${i + 1} trong Part 3 phải có nội dung`,
            });
          }
          if (!questionsPart3[i].options.every((opt) => opt.trim())) {
            return res.status(400).json({
              error: `Câu hỏi ${i + 1} trong Part 3 phải có đúng 4 lựa chọn`,
            });
          }
          if (
            !questionsPart3[i].correctAnswer ||
            !["A", "B", "C", "D"].includes(questionsPart3[i].correctAnswer)
          ) {
            return res.status(400).json({
              error: `Đáp án đúng của câu hỏi ${i + 1} trong Part 3 phải là A, B, C hoặc D`,
            });
          }
        }
      ...
      case 4:
        ...
        for (let i = 0; i < 3; i++) {
          if (!questionsPart4[i].text.trim()) {
            return res.status(400).json({
              error: `Câu hỏi ${i + 1} trong Part 4 phải có nội dung`,
            });
          }
          if (!questionsPart4[i].options.every((opt) => opt.trim())) {
            return res.status(400).json({
              error: `Câu hỏi ${i + 1} trong Part 4 phải có đúng 4 lựa chọn`,
            });
          }
          if (
            !questionsPart4[i].correctAnswer ||
            !["A", "B", "C", "D"].includes(questionsPart4[i].correctAnswer)
          ) {
            return res.status(400).json({
              error: `Đáp án đúng của câu hỏi ${i + 1} trong Part 4 phải là A, B, C hoặc D`,
            });
          }
        }
