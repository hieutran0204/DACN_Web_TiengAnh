...
const HISTORY_PATH = path.join(__dirname, '../data/layer3_band8_state.json');
...
async function runLayer3Ingestion() {
    ...
    await new Promise((resolve, reject) => {
        fs.createReadStream(CSV_PATH)
            ...
            .on('data', (row) => {
                ...
                if (score >= 8.0 && row.Essay && row.Essay.trim().length > 0) {
                    allEssays.push(row);
                }
            })
            .on('end', resolve)
    ...
    console.log(`\n🔍 Tìm thấy ${allEssays.length} bài Essay (Band 8.0+).`);
