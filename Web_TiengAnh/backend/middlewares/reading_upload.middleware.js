...
//     const uploadPath = path.join(basePath, "uploads", "reading", "image");
...
// const reading_upload = (req, res, next) => {
...
//     }
//     if (req.files?.image) {
//       req.body.image = `/uploads/reading/image/${req.files.image[0].filename}`;
//     }
...
const reading_upload = (req, res, next) => {
  upload.single("image")(req, res, (err) => {
    ...
    if (!req.file && req.body.image) {
      console.log(`📝 Giữ URL image cũ: ${req.body.image}`);
      // Không thay đổi req.body.image nếu là URL hợp lệ
    }
    if (req.file) {
      req.body.image = `/uploads/reading/image/${req.file.filename}`;
