...
export default function EditListeningQuestion() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Xử lý part
    let partId = question.part;
    if (typeof question.part === "object" && question.part?._id) {
      partId = question.part._id.toString();
    } else if (typeof question.part === "string") {
      partId = question.part;
    }
    if (!partId) {
      alert("Phần (Part) không hợp lệ.");
      return;
    }
    formData.append("part", partId);
    ...
    // Xử lý các trường theo loại câu hỏi
    if (question.type === "multiple_choice") {
      formData.append(
        "multipleChoice.question",
        question.multipleChoice?.question || ""
    ...
    if (question.type === "short_answer") {
      ...
      formData.append("shortAnswer.answer", question.shortAnswer?.answer || "");
      formData.append(
        "shortAnswer.wordLimit",
        question.shortAnswer?.wordLimit || ""
      );
