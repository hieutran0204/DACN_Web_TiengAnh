...
export default function WritingList() {
  ...
  useEffect(() => {
    ...
    const table = $(tableRef.current).DataTable({
      ...
      ajax: (data, callback) => {
        ...
        axios
          .get(
            `${backendUrl}/api/writing-questions?page=${page}&limit=${limit}`,
            {
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`${backendUrl}/api/writing-questions/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ marginBottom: 15 }}>
        ...
        <Link to="/writing/create" style={{ marginLeft: 10 }}>
          <button
            style={{
              padding: "8px 16px",
              background: "#28a745",
