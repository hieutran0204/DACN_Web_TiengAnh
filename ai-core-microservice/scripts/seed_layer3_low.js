...
const HISTORY_PATH = path.join(__dirname, '../data/layer3_ingest_state.json');
...
async function runLayer3Ingestion() {
    ...
    await new Promise((resolve, reject) => {
        fs.createReadStream(CSV_PATH)
            ...
            .on('data', (row) => {
                ...
                if (score <= 6.5 && row.Essay && row.Essay.trim().length > 0) {
                    allEssays.push(row);
                }
            })
            .on('end', resolve)
    ...
    console.log(`\n🔍 Tìm thấy ${allEssays.length} bài Essay (Band <= 6.5).`);
