...
export default function ListeningList() {
  ...
  useEffect(() => {
    ...
    const table = $(tableRef.current).DataTable({
      ...
      ajax: (data, callback) => {
        ...
        axios
          .get(
            `${backendUrl}/api/listening-questions?page=${page}&limit=${limit}`,
            {
      ...
      columns: [
        ...
        { data: null, render: (q) => q.part?.name || q.part || "-" },
        {
          ...
          render: (q) =>
            q.audio
              ? `<audio controls src="${backendUrl}${q.audio}"></audio>`
              : "-",
        },
        {
          data: null,
          render: (q) =>
            q.image
              ? `<img src="${backendUrl}${q.image}" alt="Hình" width="60"/>`
              : "-",
        },
        {
          data: null,
  ...
  const handleDetail = (id) => {
    window.open(`/listening/${id}`, "_blank");
  };

  const handleEdit = (id) => {
    window.open(`/listening/edit/${id}`, "_blank");
  };
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`${backendUrl}/api/listening-questions/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const renderers = {
    question: (q) => {
      const map = {
        ...
        form_note_table_completion: q.formNoteTableCompletion?.instruction,
        plan_map_diagram_labelling: q.planMapDiagramLabelling?.diagramUrl,
      };
      return map[q.type] || "-";
    },
    answer: (q) => {
      const map = {
        ...
        form_note_table_completion: (
          q.formNoteTableCompletion?.answers || []
        ).join(", "),
        plan_map_diagram_labelling: (
          q.planMapDiagramLabelling?.correctLabels || []
        ).join(", "),
      };
      return map[q.type] || "-";
    },
  ...
  return (
    <div style={{ padding: 20 }}>
      <h2>📋 Danh sách câu hỏi Listening</h2>
      <div style={{ marginBottom: 15 }}>
        <button onClick={logout}>Đăng xuất</button>
        <Link to="/listening/create" style={{ marginLeft: 10 }}>
