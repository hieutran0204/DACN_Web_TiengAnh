const HiddenWord = require("../../../../models/game/hiddenWord.model");
...
exports.getHiddenWordList = async (req, res) => {
  try {
    const hiddenWords = await HiddenWord.find().sort({ createdAt: -1 }).lean();
  ...
  try {
    ...
    if (newImage) {
      const oldImagePath = path.join(
        __dirname,
        "../../public/admin/img/uploads_reading_TOEIC/",
        hiddenWord.image
      );
