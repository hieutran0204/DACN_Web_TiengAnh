...
export default function AdminWordCardsPage() {
  ...
  const fetchCards = async () => {
    try {
      ...
      const response = await fetch("http://localhost:3000/api/admin/wordguessing/cards");
  ...
  const handleSubmit = async () => {
    if (!formData.topic || !formData.keyword || !formData.hintSentence || !formData.sentenceWithBlank) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      // FIX: Đổi từ /wordcards sang /cards
      const response = await fetch(`http://localhost:3000/api/admin/wordguessing/cards/${id}`, {
        method: "DELETE",
      });
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Từ cần đoán (Keyword) *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Câu gợi ý (Hint) *</label>
                  <textarea
                    value={formData.hintSentence}
                    onChange={(e) => setFormData({ ...formData, hintSentence: e.target.value })}
                    className="w-full border rounded-lg px-3 py-2"
                    rows={2}
                ...
                <div>
                  <label className="block text-sm font-medium mb-2">Câu có chỗ trống *</label>
                  <textarea
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                {filteredCards.length === 0 ? (
                  <p className="text-center text-gray-500 py-8">Chưa có card nào</p>
                ) : (
                    ...
                    <div key={card._id} className="border rounded-lg p-4">
                      ...
                      <div className="space-y-2">
                        <div>
                          ...
                          <span className="ml-2 text-lg font-bold text-blue-600">{card.keyword}</span>
                        </div>
                        <div>
                          <span className="font-semibold text-sm">Hint:</span>
                          <p className="text-gray-700 text-sm italic">{card.hintSentence}</p>
                        </div>
                        <div>
                          <span className="font-semibold text-sm">Sentence:</span>
                          <p className="text-gray-700 text-sm">{card.sentenceWithBlank}</p>
                        </div>
                      </div>
                    </div>
                  ))
