...
export default function EditVocabularyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const payload = { ...formData, words } // Warning: Backend create/update handles basic info, words might need separate recursive update if simpler API, but lets check controller. 
      // Controller updateCategory only takes basic info. Words are separate.
      // So if new, create first then add words.
      ...
      // Sync words if needed. Since updateCategory doesn't handle words array update in body according to my code, I need to call updateWords.
      // Wait, updateCategory updates { name, level, description, image }. 
      // I should update my controller to allow updating everything or handle logic here.
      // For now, I will assume I need to handle words separately or just update everything at once if I modify backend? 
      // Actually backend updateWords is specific.
      // Let's modify backend controller to allow full update OR do it here. 
      // Doing it here is safer given current backend code.
      
      // Calculate diff for existing ID? No, just "add" all missing ones?
      // Since `updateWords` allows "add" or "remove", syncing is tricky without "set".
      // I will update the backend controller to be smarter or just use a loop.
      // Actually, simple hack: Add all current words (controller checks duplicates).
      // But removing words?
      
      // Let's assume I should have implemented "setWords".
      // For now, I'll just add the new words entered in the text box.
  ...
  const handleRemoveWord = async (word: string) => {
    if (isNew) return; // Cant remove from unsaved
