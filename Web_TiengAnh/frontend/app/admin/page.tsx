...
export default function WordScrambleGame() {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch("http://localhost:3000/api/user/game/categories"); // SỬA: Full URL backend
      if (!response.ok) {
  ...
  const startGame = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`http://localhost:3000/api/user/game/random-word/${selectedCategory}`); // SỬA: Full URL
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
  ...
  const checkAnswer = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/user/game/check-answer", { // SỬA: Full URL
        method: "POST",
