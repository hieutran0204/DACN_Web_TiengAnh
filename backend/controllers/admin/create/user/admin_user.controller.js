...
exports.getUserList = async (req, res) => {
  try {
    const users = await User.find().select(
      "fullname username email phonenumber age sex role createdAt"
    );
...
exports.createUser = async (req, res) => {
  try {
    const { fullname, phonenumber, email, age, username, password, sex, role } =
      req.body;
...
exports.showEditForm = async (req, res) => {
  try {
    const user = await User.findById(req.params.id).select(
      "fullname username email phonenumber age sex role"
    );
    if (!user) {
...
exports.updateUser = async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(
      userId,
      { fullname, phonenumber, email, age, username, sex, role },
      { new: true }
    );
