...
const Exam = require("../../../../../../models/TOEIC/exam/exam.model");
...
const deleteFile = (filePath) => {
  ...
  normalizedPath = normalizedPath.replace(
    /^\/admin\/img\/Uploads_reading_TOEIC\//,
    "/shared/images/reading_TOEIC/"
  );
  normalizedPath = normalizedPath.replace(
    /^\\shared\\images\\reading_TOEIC\\/,
    "/shared/images/reading_TOEIC/"
...
exports.createQuestion = async (req, res) => {
  try {
    ...
    if (!MaCC || !TopicN || !part || !questionN || !difficulty) {
      throw new Error(
        "Vui lòng điền đầy đủ các trường bắt buộc (MaCC, TopicN, Part, Số câu hỏi, Độ khó)."
      );
    }
    ...
    if (duplicateQuestion) {
      throw new Error(
        `Câu hỏi số ${questionNumber} đã tồn tại trong Part ${partNum}, Mã đề ${topicNum}, Chứng chỉ ${MaCC}.`
      );
    }
    ...
    const questionData = {
      ...
      Img: req.file
        ? `/shared/images/reading_TOEIC/${req.file.filename}`
        : null,
    };
    ...
    switch (partNum) {
      case 5:
        if (
          ...
          throw new Error(
            "Part 5 yêu cầu câu hỏi, đúng 4 đáp án, và đáp án đúng."
          );
        }
        if (options.some((opt) => !opt?.trim())) {
          throw new Error("Đáp án Part 5 không được để trống.");
        }
        const correctAns5 = Array.isArray(correctAnswer)
          ? correctAnswer[0]
          : correctAnswer;
      ...
      case 6:
        ...
        questionData.blanks = blanks.map((b, index) => {
          ...
          if (b.options.some((opt) => !opt?.trim())) {
            throw new Error(
              `Chỗ trống ${index + 1} không được để trống đáp án.`
            );
          }
          const correctAns6 = Array.isArray(b.correctAnswer)
            ? b.correctAnswer[0]
            : b.correctAnswer;
          if (
            !correctAns6 ||
            !["A", "B", "C", "D"].includes(correctAns6.toUpperCase())
          ) {
            throw new Error(
              `Chỗ trống ${index + 1} phải có đáp án đúng A, B, C hoặc D.`
            );
          }
          return {
      ...
      case 7:
        ...
        questionData.questions = questions.map((q, index) => {
          if (
            ...
            throw new Error(
              `Câu hỏi ${index + 1} không hợp lệ: yêu cầu nội dung, 4 đáp án, và đáp án đúng.`
            );
          }
          if (!q.question.trim() || q.options.some((opt) => !opt?.trim())) {
            throw new Error(
              `Câu hỏi ${index + 1} không được để trống nội dung hoặc đáp án.`
            );
          }
          const correctAns7 = Array.isArray(q.correctAnswer)
            ? q.correctAnswer[0]
            : q.correctAnswer;
          if (!["A", "B", "C", "D"].includes(correctAns7.toUpperCase())) {
            throw new Error(
              `Câu hỏi ${index + 1} phải có đáp án đúng A, B, C hoặc D.`
            );
          }
          return {
  ...
  try {
    const exam = await Exam.findById(req.params.id)
      .populate({
        path: "questions",
        select:
          "MaCC part questionN question options correctAnswer passage blanks questions",
      })
      .lean();
