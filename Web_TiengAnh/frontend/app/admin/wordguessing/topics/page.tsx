...
export default function AdminWordTopicsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Bạn có chắc muốn xóa topic này? Tất cả cards trong topic cũng sẽ bị xóa!")) return;
    
    setIsLoading(true);
    try {
      // FIX: URL đúng theo backend routes
      const response = await fetch(`http://localhost:3000/api/admin/wordguessing/topics/${id}`, {
        method: "DELETE",
      });
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-2">Tên Topic *</label>
                  <input
                    type="text"
                    value={formData.name}
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-3">
                {topics.length === 0 ? (
                  <p className="text-center text-gray-500 py-8">Chưa có topic nào</p>
                ) : (
                  topics.map(topic => (
                    <div key={topic._id} className="border rounded-lg p-4 hover:bg-gray-50">
